...
#		upc_product_code = '2T2ZFMCA2GC001437'
        ...
        report, report_file_name = report.getReport(report_name)
        returnObject = {}
        returnObject['success'] = report
        returnObject['report_url'] = '{{base_url}}/api/download_report/' + report_file_name
        response = make_response(returnObject)
        response.headers["Content-Type"] = "applicationlication/pdf"
        response.headers["Content-Disposition"] = "inline; filename=output.pdf"
        return response, 200
...
@application.route('/api/download_report/<report_name>')
def downloadReport(report_name):
    path = './ReportService/Example PDF/'
    return send_from_directory(path, report_name)
